...
private:
    ...
    bool is_using_inline_storage() const noexcept {
...
public:
    ...
    const_reference operator[](size_type pos) const noexcept { return begin_[pos]; }
    ...
    size_type capacity() const noexcept { return capacity_end_ - begin_; }
    ...
    void shrink_to_fit() {
        if (!is_using_inline_storage() && size() <= N) {
    ...
    reference emplace_back(Args&&... args) {
        ...
        std::allocator_traits<Allocator>::construct(alloc_, end_, 
    ...
    void swap(small_vector& other) noexcept {
        if (is_using_inline_storage() && other.is_using_inline_storage()) {
            ...
            if (size() < other.size()) {
                std::uninitialized_move_n(other.begin_ + size(), 
                    other.size() - size(), end_);
                destroy_range(other.begin_ + size(), other.end_);
            } else if (other.size() < size()) {
                std::uninitialized_move_n(begin_ + other.size(), 
                    size() - other.size(), other.end_);
    ...
    // Range support
    auto as_span() noexcept { return std::span(begin_, end_); }
    auto as_span() const noexcept { return std::span(begin_, end_); }
    ...
    std::string debug_string() const {
        return std::format("small_vector[size={}, capacity={}, inline={}]",
            size(), capacity(), is_using_inline_storage());
    }
    ...
    void transform_inplace(UnaryOp op) {
        std::transform(begin(), end(), begin(), std::move(op));
    }

    template<typename BinaryOp>
    void transform_inplace(const small_vector& other, BinaryOp op) {
    ...
    void insert_bulk(const_iterator pos, InputIt first, InputIt last) {
        ...
        if constexpr (std::contiguous_iterator<InputIt> && 
            ...
            if (const auto move_count = end_ - insert_pos; move_count > 0) {
                std::memmove(insert_pos + count, insert_pos, 
                    move_count * sizeof(T));
            }
    ...
    void reserve_exact(size_type new_cap) {
        ...
        if constexpr (std::is_trivially_copyable_v<T>) {
            new_begin = static_cast<pointer>(
                std::aligned_alloc(alignment_helper::value, 
                                 new_cap * sizeof(T)));
            if (!new_begin) throw std::bad_alloc();
    ...
    void stable_partition_inplace(Pred pred) {
    ...
    memory_stats get_memory_stats() const noexcept {
        ...
        return memory_stats{
            ...
            .inline_capacity = N,
            .using_inline_storage = is_using_inline_storage(),
    ...
    // 指针类型的概念
    template<typename U>
    concept pointer_like = std::is_pointer_v<U> || 
        requires(U u) { 
            { u.operator->() } -> std::convertible_to<void*>;
            { *u } -> std::convertible_to<void>;
        };

    // 可删除的指针概念
    template<typename U>
    concept deletable_pointer = pointer_like<U> && 
        requires(U* p) {
            delete p;
        };
...
struct simd_traits<int32_t> {
    static constexpr bool is_vectorizable = true;
    static constexpr size_t vector_size = 8;  // AVX2
    using vector_type = __m256i;
};

// small_vector_view class
template<typename T>
...
public:
    ...
    constexpr small_vector_view(T* data, size_t size) noexcept
    ...
    // Range support
    auto as_span() noexcept { return std::span(data_, size_); }
    auto as_span() const noexcept { return std::span(data_, size_); }
};
...
bool operator==(const small_vector<T, N, Alloc>& lhs, 
                const small_vector<T, N, Alloc>& rhs) {
    return std::equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
}

template<typename T, std::size_t N, typename Alloc>
auto operator<=>(const small_vector<T, N, Alloc>& lhs, 
                 const small_vector<T, N, Alloc>& rhs) {
...
template<typename T, std::size_t N, typename Alloc>
void swap(small_vector<T, N, Alloc>& lhs, 
         small_vector<T, N, Alloc>& rhs) noexcept {
    lhs.swap(rhs);
}
