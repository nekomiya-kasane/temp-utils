...
template<> class UniqueId<uint128_t> {
 public:
  using value_type = uint128_t;
  static constexpr size_t size = 16;

  // Default constructor generates a random UUID
  UniqueId() : _value(generate()._value) {}

  // Construct from high and low parts
  template<typename U>
    requires std::convertible_to<U, uint64_t>
  constexpr UniqueId(U high, U low)
      : _value(uint128_t(static_cast<uint64_t>(high), static_cast<uint64_t>(low)))
  {
  }

  // Construct from 8-4-4-4-12 format components
  constexpr UniqueId(uint32_t time_low,
                     uint16_t time_mid,
                     ...
                     uint64_t node)
  {
    ...
    uint64_t low = (static_cast<uint64_t>(clock_seq) << 48) | (node & 0x0000FFFFFFFFFFFFULL);

    _value = uint128_t(high, low);
  }
  ...
  explicit constexpr UniqueId(const UniqueId<U, S> &other)
  {
    ...
    _value = uint128_t(0);
    std::copy_n(bytes.begin(), S, reinterpret_cast<uint8_t *>(&_value));
  }
  ...
  explicit constexpr operator UniqueId<U, S>() const
  {
    ...
    std::copy_n(src_bytes.begin(), S, reinterpret_cast<uint8_t *>(&result._value));
    return result;
  }
  ...
  std::array<UniqueId<U, S>, (size + S - 1) / S> split() const
  {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
      ...
      std::copy_n(
          src_bytes.begin() + i * S, copy_size, reinterpret_cast<uint8_t *>(&result[i]._value));
    }
  ...
  constexpr uint32_t component_1() const
  {
    return static_cast<uint32_t>(_value.high >> 32);
  }

  constexpr uint16_t component_2() const
  {
    return static_cast<uint16_t>(_value.high >> 16);
  }

  constexpr uint16_t component_3() const
  {
    return static_cast<uint16_t>(_value.high);
  }

  constexpr uint16_t component_4() const
  {
    return static_cast<uint16_t>(_value.low >> 48);
  }

  constexpr uint64_t component_5() const
  {
    return _value.low & 0x0000FFFFFFFFFFFFULL;
  }
  ...
  constexpr bool operator==(const UniqueId &) const = default;
  ...
  /*constexpr*/ std::array<uint8_t, size> bytes() const
  {
    ...
    const uint8_t *ptr = reinterpret_cast<const uint8_t *>(&_value);
  ...
  bool setFromHex(std::string_view hex)
  {
    ...
    for (size_t i = 0; i < 8; ++i) {
      uint8_t high_nibble = misc::hex_to_int(cleaned[i * 2]);
      uint8_t low_nibble = misc::hex_to_int(cleaned[i * 2 + 1]);
  ...
  std::string format(const detail::format_spec &spec = {}) const
  {
    ...
    if (spec.use_dashes)
      required_size += 4;  // For UUID dashes
...
using Uuid = UniqueId<uint128_t, 16>;

// Formatter specialization for UUID
template<> struct std::formatter<UniqueId<uint128_t, 16>> : std::formatter<std::string> {
  template<typename FormatContext> auto format(const UniqueId<uint128_t> &id, FormatContext &ctx)
  {
